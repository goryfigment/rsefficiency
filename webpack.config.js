...
module.exports = {
    ...
    plugins: [
        ...
        new PurifyCSSPlugin({paths: glob.sync([path.join(__dirname, 'templates/*.html'), path.join(__dirname, 'templates/partials/*.html')]), minimize: true,
            purifyOptions: {whitelist: ['active', 'selected', 'index', 'positive', 'negative', 'neutral', 'question', 'multiple', 'ingredient', 'substitute-cost', 'subheaders', '*-img*', 'exp-wrapper', 'highscore-wrapper', 'calculator-options',
                'next-level', 'next-level-title', 'next-level-container', 'highscore-submit', 'combat-submit', 'highscore-username', 'highscore-label', 'highscore-wrapper', 'exp-input', "output_wrapper", 'infinite', '*pure*']}
        }),
